...
// ▼▼▼ BẠN CHÈN ĐOẠN NÀY VÀO NGAY DƯỚI ▼▼▼

// --- KHẮC PHỤC LỖI MẤT MENU ---
// Kiểm tra xem trong kho có dữ liệu chưa?
let dataTrongKho = localStorage.getItem('products');

if (dataTrongKho) {
    // Nếu có (do Admin sửa), thì lấy ra dùng
    products = JSON.parse(dataTrongKho);
} else {
    // Nếu chưa có (hoặc vừa bị xóa), thì LƯU NGAY danh sách mặc định vào kho
    // Để lần sau Admin mở lên sẽ thấy danh sách này chứ không phải trang trắng
...
// ⭐ THAY THẾ HÀM checkLoginStatus() CŨ BẰNG HÀM NÀY
...
    const welcomeUser = document.getElementById('welcomeUser'); 
    ...
    // ⭐ SỬA: Lấy 2 link Admin bằng ID mới
...
        if (role === 'admin') {
            // ⭐ SỬA: Làm hiện CẢ HAI link
            if(revenueLink) {
                revenueLink.style.display = 'inline-block'; 
            }
            if(adminPageLink) {
                adminPageLink.style.display = 'inline-block'; 
            }
            if(adminControls) {
...
    
    if (name && price) {
        // Giả định 'products' và 'renderProducts' đã tồn tại ở đâu đó trong file
...
    
    if (name && price) {
        // Giả định products và renderProducts đã tồn tại ở đâu đó
        products.push({ name: name, price: price }); 
        renderProducts();
        alert(`Thêm sản phẩm "${name}" thành công!`);
        if (nameInput) nameInput.value = '';
...
document.getElementById('cartIcon').addEventListener('click', () => {
    ...
    if (cart.length === 0) {
        ...
        payBtn.id = "btnThanhToanNgay"; // Gọi hàm thanh toán
...
function thanhToan() {
    ...
    // 1. Lấy tên khách hàng (nếu đã đăng nhập)
    ...
    let newOrder = {
        id: madon,
        user: currentUser,
        item: orderDetails.substring(0, 30) + "...", // Cắt ngắn nếu dài quá
        total: totalItems * 40 + ".000đ", // Tính tạm giá trung bình 40k/ly
        status: "pending" // Mặc định là đang chờ
    };

    // 6. Lấy danh sách đơn hàng cũ từ kho ra
    let listDonHang = JSON.parse(localStorage.getItem('listDonHang')) || [];
    
    // 7. Thêm đơn mới vào đầu danh sách
    listDonHang.unshift(newOrder);

    // 8. Lưu lại vào kho
    localStorage.setItem('listDonHang', JSON.stringify(listDonHang));

    // 9. Xóa giỏ hàng và thông báo
    cart = [];
...
function dongCuaSo() {
    // Tìm cái khung theo ID chính xác là 'cartPopup'
    var khung = document.getElementById("cartPopup");
    if (khung) {
...
// ========== HÀM ĐÓNG GIỎ HÀNG ========== //
function dongCuaSo() {
    // Tìm khung giỏ hàng theo đúng ID trong HTML
    var khungGioHang = document.getElementById("cartPopup");
